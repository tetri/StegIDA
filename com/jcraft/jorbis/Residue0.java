...
class Residue0 extends FuncResidue {
	 ...
	 * //System.err.println("Residue0.inverse"); int
	 * i,j,k,l,transend=vb.pcmend/2; LookResidue0 look=(LookResidue0 )vl;
	 * InfoResidue0 info=look.info;
	 *  // move all this setup out later int
	 * samples_per_partition=info.grouping; int
	 * partitions_per_word=look.phrasebook.dim; int n=info.end-info.begin;
	 * 
	 * int partvals=n/samples_per_partition; int
	 * partwords=(partvals+partitions_per_word-1)/partitions_per_word; int[][]
	 * partword=new int[ch][]; float[] work=new float[samples_per_partition];
	 * partvals=partwords*partitions_per_word;
	 *  // make sure we're zeroed up to the start for(j=0;j<ch;j++){ for(k=0; k<info.begin;
	 * k++)in[j][k]=0.0f; }
	 * 
	 * for(i=info.begin,l=0;i<info.end;){ // fetch the partition word for each
	 * channel for(j=0;j<ch;j++){ int temp=look.phrasebook.decode(vb.opb);
	 * if(temp==-1){ //goto eopbreak; if(i<transend){ for(j=0;j<ch;j++){
	 * for(k=0;k<transend-i;k++)in[j][i+k]=0.0f; } } return(0); }
	 * partword[j]=look.decodemap[temp]; if(partword[j]==null){ //goto errout;
	 * for(j=0;j<ch;j++){ for(k=0;k<transend;k++)in[j][k]=0.0f; } return(0); } }
	 *  // now we decode interleaved residual values for the partitions
	 * for(k=0;k<partitions_per_word;k++,l++,i+=samples_per_partition){
	 * for(j=0;j<ch;j++){ int part=partword[j][k]; if(decodepart(vb.opb,work,
	 * in[j], i,samples_per_partition, info.secondstages[part],
	 * look.partbooks[part])==-1){ //goto eopbreak; if(i<transend){ for(j=0;j<ch;j++){
	 * for(k=0;k<transend-i;k++)in[j][i+k]=0.0f; } } return(0); } } } }
	 *  // eopbreak: if(i<transend){ for(j=0;j<ch;j++){ for(k=0;k<transend-i;k++)in[j][i+k]=0.0f; } }
	 * return(0);
	 *  // errout: // for(j=0;j<ch;j++) // for(k=0;k<transend;k++)in[j][k]=0.0f; //
	 * return(0); } int decodepart(Buffer opb, float[] work, float[] vec, int
	 * veci, int n, int stages, CodeBook[] books){ int i,j; for(i=0;i<n;i++)work[i]=0.0f;
	 * 
	 * for(j=0;j<stages;j++){ int dim=books[j].dim; int step=n/dim; for(i=0;i<step;i++){
	 * if(books[j].decodevs(work, i, opb, step, 0)==-1){ return(-1); } } }
	 * for(i=0;i<n;i++){ vec[veci+i]*=work[i]; } return(0); }
	 */
