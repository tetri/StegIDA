...
class Mapping0 extends FuncMapping {
	...
	void free_look(Object imap) {
		/*
		 * LookMapping0 l=(LookMapping0)imap; InfoMapping0 info=l.map;
		 * if(l!=null){ for(int i=0;i<l.map.submaps;i++){
		 * l.time_func[i].free_look(l.time_look[i]);
		 * l.floor_func[i].free_look(l.floor_look[i]);
		 * l.residue_func[i].free_look(l.residue_look[i]);
		 * if(l.psy_look!=null)l.psy_look[i].clear(); } }
		 * 
		 * if(l.floor_state!=null){ for(int i=0;i<l.ch;i++)
		 * l.floor_func[info.chmuxlist[i]].free_state(l.floor_state[i]);
		 * //free(l.floor_state); }
		 * 
		 * if(l.decay!=null){ for(int i=0;i<l.ch;i++){
		 * //if(l.decay[i])free(l->decay[i]); l.decay[i]=null; }
		 * //free(l->decay); l.decay=null; } //free(l->time_func);
		 * //free(l->floor_func); //free(l->residue_func); //free(l->time_look);
		 * //free(l->floor_look); //free(l->residue_look);
		 * //f(l->psy_look)free(l->psy_look); l.time_func=null;
		 * l.floor_func=null; l.residue_func=null; l.time_look=null;
		 * l.floor_look=null; l.residue_look=null;
		 * //memset(l,0,sizeof(vorbis_look_mapping0)); //free(l);
		 */
	}
	...
	Object look(DspState vd, InfoMode vm, Object m) {
		...
		/*
		 * if(vd.analysisp!=0){ look.floor_state=new Object[vi.channels]; }
		 * if(vi.psys!=0){ look.psy_look=new PsyLook[info.submaps]; for(int i=0;
		 * i<info.submaps; i++){ look.psy_look[i]=new PsyLook(); } }
		 */
		...
		for (int i = 0; i < info.submaps; i++) {
			...
			/*
			 * if(vi.psys!=0 && vd.analysisp!=0){ int psynum=info.psysubmap[i];
		...
		if (vi.psys != 0 && vd.analysisp != 0) {
			 ...
			 * ci->psy_g_param, ci->blocksizes[vm->blockflag]/2,vi->rate);
			 * 
			 * psynum=info->psy[1];
	...
	/*
	 * // no time mapping implementation for now static int seq=0; int
	 * forward(Block vb, Object l){ DspState vd=vb.vd; Info vi=vd.vi;
	 * LookMapping0 look=(LookMapping0)l; InfoMapping0 info=look.map; InfoMode
	 * mode=look.mode; int n=vb.pcmend; float[]
	 * window=vd.window[vb.W][vb.lW][vb.nW][mode.windowtype];
	 * 
	 * float[][] pcmbundle=new float[vi.channles][]; int[] nonzero=new
	 * int[vi.channels];
	 *  // time domain pre-window: NONE IMPLEMENTED
	 *  // window the PCM data: takes PCM vector, vb; modifies PCM vector
	 * 
	 * for(int i=0;i<vi.channels;i++){ float[] pcm=vb.pcm[i]; for(int j=0;j<n;j++)
	 * pcm[j]*=window[j]; }
	 *  // time-domain post-window: NONE IMPLEMENTED
	 *  // transform the PCM data; takes PCM vector, vb; modifies PCM vector //
	 * only MDCT right now.... for(int i=0;i<vi.channels;i++){ float[]
	 * pcm=vb.pcm[i]; mdct_forward(vd.transform[vb.W][0],pcm,pcm); }
	 *  { float[] floor=_vorbis_block_alloc(vb,n*sizeof(float)/2);
	 * 
	 * for(int i=0;i<vi.channels;i++){ float[] pcm=vb.pcm[i]; float[]
	 * decay=look.decay[i]; int submap=info.chmuxlist[i];
	 *  // if some other mode/mapping was called last frame, our decay //
	 * accumulator is out of date. Clear it. //if(look.lastframe+1 !=
	 * vb->sequence) // memset(decay,0,n*sizeof(float)/2);
	 ...
	 *  // perform floor encoding nonzero[i]=look.floor_func[submap].
	 ...
	 * multiplexed. All the channels belonging to one submap are // encoded
	 * (values interleaved), then the next submap, etc
	 * 
	 * for(int i=0;i<info.submaps;i++){ int ch_in_bundle=0; for(int j=0;j<vi.channels;j++){
	 * if(info.chmuxlist[j]==i && nonzero[j]==1){
	 * pcmbundle[ch_in_bundle++]=vb.pcm[j]; } }
	 * look.residue_func[i].forward(vb,look.residue_look[i],
	 * pcmbundle,ch_in_bundle); } } look.lastframe=vb.sequence; return(0); }
	 */
