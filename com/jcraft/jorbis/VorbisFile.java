...
public class VorbisFile {
	...
	private int get_data() {
		...
		byte[] buffer = oy.data;
		// int bytes=callbacks.read_func(buffer, index, 1, CHUNKSIZE,
		// datasource);
		int bytes = 0;
		try {
	...
	private int get_prev_page(Page page) throws JOrbisException {
		...
		if (ret < 0) {
			// System.err.println("Missed page fencepost at end of logical
			// bitstream Exiting");
			// System.exit(1);
			return OV_EFAULT;
		}
		...
		while (i < 3) {
			...
			while (i < 3) {
				...
				if (result == 0)
					break;
				if (result == -1) {
					// System.err.println("Corrupt header in logical
					// bitstream.");
					// goto bail_header;
					vi.clear();
					vc.clear();
				...
				if (vi.synthesis_headerin(vc, op) != 0) {
					// System.err.println("Illegal header in logical
					// bitstream.");
					// goto bail_header;
					vi.clear();
					vc.clear();
			...
			if (i < 3)
				if (get_next_page(og_ptr, 1) < 0) {
					// System.err.println("Missing header in logical
					// bitstream.");
					// goto bail_header;
					vi.clear();
					vc.clear();
			...
			// get the last page of the stream
			{
				...
				while (true) {
					...
					if (ret == -1) {
						...
						// System.err.println("Could not find last page of
						// logical "+
						// "bitstream #"+(i)+"\n");
						vi[i].clear();
						vc[i].clear();
	...
	/*
	 * public static void main(String[] arg){ try{ VorbisFile foo=new
	 * VorbisFile(arg[0]); int links=foo.streams();
	 * System.out.println("links="+links); Comment[] comment=foo.getComment();
	 * Info[] info=foo.getInfo(); for(int i=0; i<links; i++){
	 * System.out.println(info[i]); System.out.println(comment[i]); }
	 * System.out.println("raw_total: "+foo.raw_total(-1));
	 * System.out.println("pcm_total: "+foo.pcm_total(-1));
	 * System.out.println("time_total: "+foo.time_total(-1)); } catch(Exception
