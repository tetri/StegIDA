...
public class DspState {
...

		{
			...
			if (granulepos == -1) {
				...
				if (vb.granulepos != -1 && granulepos != vb.granulepos) {
					...
					}// else{ Shouldn't happen *unless* the bitstream is out
						// of
	...
	public void clear() {
		/*
		 * if(window[0][0][0]!=0){ for(i=0;i<VI_WINDOWB;i++)
		 * if(v->window[0][0][0][i])free(v->window[0][0][0][i]);
		 * free(v->window[0][0][0]);
		 * 
		 * for(j=0;j<2;j++) for(k=0;k<2;k++){ for(i=0;i<VI_WINDOWB;i++)
		 * if(v->window[1][j][k][i])free(v->window[1][j][k][i]);
		 * free(v->window[1][j][k]); } }
		 * 
		 * if(v->pcm){ for(i=0;i<vi->channels;i++)
		 * if(v->pcm[i])free(v->pcm[i]); free(v->pcm);
		 * if(v->pcmret)free(v->pcmret); }
		 * if(v->multipliers)free(v->multipliers);
		 * 
		 * _ve_envelope_clear(&v->ve); if(v->transform[0]){
		 * mdct_clear(v->transform[0][0]); free(v->transform[0][0]);
		 * free(v->transform[0]); } if(v->transform[1]){
		 * mdct_clear(v->transform[1][0]); free(v->transform[1][0]);
		 * free(v->transform[1]); }
		 *  // free mode lookups; these are actually vorbis_look_mapping structs
		 * if(vi){ for(i=0;i<vi->modes;i++){ int
		 * mapnum=vi->mode_param[i]->mapping; int maptype=vi->map_type[mapnum];
		 * _mapping_P[maptype]->free_look(v->mode[i]); } // free codebooks
		 * for(i=0;i<vi->books;i++) vorbis_book_clear(v->fullbooks+i); }
		 * 
		 * if(v->mode)free(v->mode); if(v->fullbooks)free(v->fullbooks);
		 *  // free header, header1, header2 if(v->header)free(v->header);
		 * if(v->header1)free(v->header1); if(v->header2)free(v->header2);
		 * 
		 * memset(v,0,sizeof(vorbis_dsp_state)); }
		 */
	}
