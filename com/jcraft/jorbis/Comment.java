...
public class Comment {
	...
	/*
	 * private void add_tag(byte[] tag, byte[] contents){ byte[] foo=new
	 * byte[tag.length+contents.length+1]; int j=0; for(int i=0; i<tag.length;
	 * i++){foo[j++]=tag[i];} foo[j++]=(byte)'='; j++; for(int i=0; i<contents.length;
	 * i++){foo[j++]=tag[i];} add(foo); }
	 */
	...
	public Properties getCommentsAsProperties() {
		Properties props = new Properties();
		String[] keys = new String[comments];
		String[] values = new String[comments];
		for (int i = 0; i < comments; i++) {
			keys[i] = new String();
			values[i] = new String();
			String comment;
			try {
				comment = new String(user_comments[i], 0,
						user_comments[i].length - 1, "UTF-8");
			} catch (UnsupportedEncodingException e) {
				comment = new String(user_comments[i], 0,
			...
			props.put(keys[i].toUpperCase(), values[i]);
		}
		return props;
	}
