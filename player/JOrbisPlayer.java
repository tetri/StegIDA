...
public class JOrbisPlayer extends JApplet implements ActionListener, Runnable {
	...
	private void play_udp_stream(Thread me) {
		 ...
		 * oy.init();
		 * 
		 * loop: while(true){ int eos=0; int index=oy.buffer(BUFSIZE);
		 * buffer=oy.data; try{ bytes=io.getByte(buffer, index, BUFSIZE); }
		 * catch(Exception e){ System.err.println(e); return; } oy.wrote(bytes);
		 * if(!firstloop){ if(oy.pageout(og)!=1){ if(bytes<BUFSIZE)break;
		 * System.err.println("Input does not appear to be an Ogg bitstream.");
		 * return; }
		 * 
		 * os.init(og.serialno()); os.reset();
		 * 
		 * vi.init(); vc.init();
		 * 
		 ...
		 * System.err.println("Error reading initial header packet."); break; //
		 * return; }
		 * 
		 * if(vi.synthesis_headerin(vc, op)<0){ // error case; not a vorbis
		 * header System.err.println("This Ogg bitstream does not contain Vorbis
		 * audio data."); return; }
		 * 
		 * int i=0; while(i<2){ while(i<2){ int result=oy.pageout(og);
		 * if(result==0) break; // Need more data if(result==1){ os.pagein(og);
		 * while(i<2){ result=os.packetout(op); if(result==0)break;
		 * if(result==-1){ System.err.println("Corrupt secondary header.
		 * Exiting."); //return; break loop; } vi.synthesis_headerin(vc, op);
		 * i++; } } }
		 * 
		 * index=oy.buffer(BUFSIZE); buffer=oy.data; try{
		 * bytes=io.getByte(buffer, index, BUFSIZE); } catch(Exception e){
		 * System.err.println(e); return; } if(bytes==0 && i<2){
		 * System.err.println("End of file before finding all Vorbis headers!");
		 * return; } oy.wrote(bytes); } } else{ firstloop=false; }
		 *  { byte[][] ptr=vc.user_comments; StringBuffer sb=null;
		 * if(acontext!=null) sb=new StringBuffer();
		 * 
		 * for(int j=0; j<ptr.length;j++){ if(ptr[j]==null) break;
		 * System.err.println("Comment: "+new String(ptr[j], 0,
		 * ptr[j].length-1)); if(sb!=null)sb.append(" "+new String(ptr[j], 0,
		 * ptr[j].length-1)); } System.err.println("Bitstream is "+vi.channels+"
		 * channel, "+vi.rate+"Hz"); System.err.println("Encoded by: "+new
		 * String(vc.vendor, 0, vc.vendor.length-1)+"\n");
		 * if(sb!=null)acontext.showStatus(sb.toString()); }
		 * 
		 * convsize=BUFSIZE/vi.channels;
		 * 
		 * vd.synthesis_init(vi); vb.init(vd);
		 * 
		 * double[][][] _pcm=new double[1][][]; float[][][] _pcmf=new
		 * float[1][][]; int[] _index=new int[vi.channels];
		 * 
		 * getOutputLine(vi.channels, vi.rate);
		 * 
		 * while(eos==0){ while(eos==0){
		 * 
		 * if(player!=me){ //System.err.println("bye."); try{
		 * //outputLine.drain(); //outputLine.stop(); //outputLine.close();
		 * io.close(); } catch(Exception ee){} return; }
		 * 
		 * int result=oy.pageout(og); if(result==0)break; // need more data
		 * if(result==-1){ // missing or corrupt data at this page position //
		 * System.err.println("Corrupt or missing data in io; continuing..."); }
		 * else{ os.pagein(og); while(true){ result=os.packetout(op);
		 * if(result==0)break; // need more data if(result==-1){ // missing or
		 * corrupt data at this page position // no reason to complain; already
		 * complained above } else{ // we have a packet. Decode it int samples;
		 * if(vb.synthesis(op)==0){ // test for success!
		 * vd.synthesis_blockin(vb); } while((samples=vd.synthesis_pcmout(_pcmf,
		 * _index))>0){ double[][] pcm=_pcm[0]; float[][] pcmf=_pcmf[0]; boolean
		 * clipflag=false; int bout=(samples<convsize?samples:convsize);
		 *  // convert doubles to 16 bit signed ints (host order) and //
		 * interleave for(int i=0;i<vi.channels;i++){ int ptr=i*2; //int ptr=i;
		 * int mono=_index[i]; for(int j=0;j<bout;j++){ int
		 * val=(int)(pcmf[i][mono+j]*32767.); if(val>32767){ val=32767;
		 * clipflag=true; } if(val<-32768){ val=-32768; clipflag=true; } if(val<0)
		 * val=val|0x8000; convbuffer[ptr]=(byte)(val);
		 * convbuffer[ptr+1]=(byte)(val>>>8); ptr+=2*(vi.channels); } }
		 * outputLine.write(convbuffer, 0, 2*vi.channels*bout);
		 * vd.synthesis_read(bout); } } } if(og.eos()!=0)eos=1; } }
		 * 
		 * if(eos==0){ index=oy.buffer(BUFSIZE); buffer=oy.data; try{
		 * bytes=io.getByte(buffer,index,BUFSIZE); } catch(Exception e){
		 * System.err.println(e); return; } if(bytes==-1) break;
		 * oy.wrote(bytes); if(bytes==0)eos=1; } } os.clear(); vb.clear();
		 * vd.clear(); vi.clear(); }
		 * 
		 * oy.clear();
		 * 
